...
<script type="text/javascript">
	...
	$(function() {
				...
				.uploadify(
						{
							...
							'onComplete' : function(event, queueID, fileObj,
								...
								if (result.flag) {
									alertMsg.correct("文件:" + fileObj.name
											+ " 上传成功!");
									reloadUrgencyTab();
								} else {
	    ...
	    $("#urgencyImgUpload").ajaxStop(function(){
	    	ajaxbg.hide();
											...
											.ajax({
												...
												success : function(data) {
													...
													if (!checkEditEnable(data.status)) {
														$("#adPackageStatusAddUploadUrgency").val(data.status);
														reloadUrgencyTab();
		...
		//取消
		$("#btnCancelUrgencyAdd").click(function(){
	...
	function reloadUrgencyTab() {
		navTab
				.reload(
						"ad/adPackage!toUploadPicUrgencyList.do?adPackage.adPackageId=<s:property value="adResource.adPackageId"/> && adStyle=<s:property value="adStyle"/>&adPackage.adPackageStatus="+$("#adPackageStatusAddUploadUrgency").val(),
						{
							navTabId : "toAddUrgency"
						});
		navTab.switchTab("toAddUrgency");
	}
...
<div class="pageContent">
	<s:form id="urgencyImgUpload" theme="simple" method="post"
		...
		<s:hidden name="adPackage.adPackageStatus" id="adPackageStatusAddUploadUrgency"/>
		...
		<div class="formBar">
			<ul>
				<li><a class="button" id="saveUrgencyBtn" width="520"
					height="430" title="保存"><span>保存</span></a></li>
				<li><a class="button" id="btnCancelUrgencyAdd" width="520"
					height="430" title="取消"><span>取消</span></a></li>
			</ul>
		</div>
	</s:form>
</div>
